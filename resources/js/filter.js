...
//fixOptionArrays.
    ...
    function RolesAuth() {
        ...
        self.effectiveDateFormatted = ko.observable();
        self.expirationDateFormatted = ko.observable();
        self.activityDtTm = ko.observable();

        self.effectiveDate = ko.computed(function () {
            return self.effectiveDateFormatted() == null ? null : moment(self.effectiveDateFormatted()).toDate();
        }, self);

        self.expirationDate = ko.computed(function () {
            return self.expirationDateFormatted() == null ? null : moment(self.expirationDateFormatted()).toDate();
        }, self);
        ...
        self.init = function (rolesAuthEntity) {
            self.userId(rolesAuthEntity.userId);
            self.name(rolesAuthEntity.name == null ? "No Name Found" : rolesAuthEntity.name);
            self.applicationCode(rolesAuthEntity.applicationCode);
            self.role(rolesAuthEntity.role);
            self.qualifier(rolesAuthEntity.qualifier);
            self.hierarchy(rolesAuthEntity.hierarchy);
            self.effectiveDateFormatted(rolesAuthEntity.effectiveDate == null ? null : moment(rolesAuthEntity.effectiveDate).format("MM/DD/YYYY"));
            self.expirationDateFormatted(rolesAuthEntity.expirationDate == null ? null : moment(rolesAuthEntity.expirationDate).format("MM/DD/YYYY"));
            self.activityDtTm(rolesAuthEntity.activityDtTm);
        };

        self.initFromRolesAuth = function (rolesAuth) {
            self.userId(rolesAuth.userId());
            self.name(rolesAuth.name());
            self.applicationCode(rolesAuth.applicationCode());
            self.role(rolesAuth.role());
            self.qualifier(rolesAuth.qualifier());
            self.hierarchy(rolesAuth.hierarchy());
            self.effectiveDateFormatted(rolesAuth.effectiveDateFormatted());
            self.expirationDateFormatted(rolesAuth.expirationDateFormatted());
            self.activityDtTm(rolesAuth.activityDtTm());
        };

        self.getToSave = function () {
            return {
                userId: self.userId(),
                ...
                activityDtTm: self.activityDtTm()
            };
        };

        self.equals = function (compareToRolesAuth) {
            return self.userId() == compareToRolesAuth.userId() &&
                self.applicationCode() == compareToRolesAuth.applicationCode() &&
                self.role() == compareToRolesAuth.role();
        }
    };
